...
- `config.js` : `CONFIG_PAR_DEFAUT` (persos, armes, modes, maps, rôles, gadgets, quêtes, rangs, skins, `app` = réglages généraux) + `migrerConfig(c)` : mises à jour numérotées (`c.version`, actuellement **24** (les nouvelles listes de config — combos, météos, raretés, pass… — sont ajoutées automatiquement si absentes, via la liste de clés en tête de `migrerConfig`)). Toute nouvelle donnée ⇒ nouveau bloc `if ((c.version || 0) < N)` placé **après** le dernier.
...
Grande carte « 🏰 Royaume » (64×64, générée : 6 ambiances en secteurs + place neutre au centre, `map.def.biomes`, `biomeA(tx,ty)`, rendu 3D par ambiance, reconstruction 3D limitée sur les grandes cartes) et mode « Royaume » (survie ; construction (casser → matériaux `j.mat`, `construireMur`, touche F / bouton 🧱, réglages coutMur/matMur/solideMur…) ; butin (`mode.butin`, coffres → `A:arme:rareté`, `CONFIG.raretes`, `ramasser`, `multButin`, perdu au K.O.) ; atterrissage en parachute (`mode.atterrissage`, `chuteK()` / `enChute()`, intouchable et sans tir pendant la chute, parachute 3D) ; combos d'éléments (`CONFIG.combos` « a+b » triés, `comboElem`, déclenchés dans `degats` quand 2 éléments différents touchent la même cible en < `comboDelai` s) ; météo (`CONFIG.meteos`, `choisirMeteo`, `meteoMult(k)`, `dessinerMeteo`, visibilité réduite via `visible()`) ; boss mondial (`mode.bossMondial`, `majBossMondial` chez l'hôte, `b.mondial` synchronisé, lâche des armes légendaires via l'évènement `bl`) ; planche manga de fin de partie (`genererPlanche` : 4 cases dessinées sur un canvas 900×1260, bouton « 📖 Ma planche », partage / enregistrement) ; pass de saison (`CONFIG.pass` : XP par victoire/défaite/K.O./quête, 20 paliers de récompenses dont des skins, `passId/passXP/passPris` dans le profil, remis à zéro chaque mois) — **fusionné avec les récompenses de carrière** : un seul bouton « Récompenses » avec onglets ⭐ Saison / 🏆 Carrière (`ongletsRec`)) ; écran VS de l'intro : les persos en 3D figés en pose d'attaque (`equipe3D`, une photo par perso), groupés par équipe ; planche de fin avec la photo de profil (+ médaillon du perso)).
Persos : ROKH, ZEPHYR, NAIA, PYRO (de base), BOULDO (tireur) et **NIMBUS** (ajouté en v24 : air, Éventail de vent en lame, vol, Tornade, rôle contrôle, gadget Rafale, danse `Bubble_Dance`, chute `Fall1`). Un nouveau modèle déposé dans `modeles/` doit être ajouté à la config (bloc de migration) pour apparaître en ligne ; vérifier ses animations en lisant le JSON du .glb.
Admin › **Joueurs** : tout le profil de chaque joueur est modifiable (points, victoires, parties, jetons, points de saison, XP du pass, essences, persos débloqués (`persosDebloques` = `cleP(perso)`), skins par perso, niveau/points par perso) — **enregistrement automatique** à chaque changement (≠ « Publier », qui n'enregistre que la config du jeu).
Animation de **chute** (parachute) : `anims.chute` dans `modele3d.js` (nom avec « fall » ou champ `animChute`), jouée dans `rendu3d.js` pendant `chuteK() > 0`.
**Barre du haut des menus** : `barreRessources` — grosses cases Trophées / Jetons perso / Essences (par élément), cliquables (classement, persos) ; rétrécit sur petit écran.
**Blancs éclatants** : dans `modele3d.js`, `blancs(sh)` (patch du shader toon, aussi appliqué avec les skins) garde les zones très claires de la texture bien blanches ; réglage admin `eclatBlancs` (0 à 1, défaut 0.7).
**Marathon sur plusieurs cartes** : chaque épreuve a sa carte (`mode.cartesEtapes` = « bloc:0,zone:3 », choisie dans l'admin à côté de chaque épreuve ; sinon auto : une carte active qui a ce qu'il faut — zone `Z`, tours, cachettes `O`). Étape gagnée → `ouvrirChemin` : portail au centre + flèches (`dessinerChemin`, `hudChemin`), `obj()` = 'route' ; quand tous les humains y sont entrés (évènement `ce`) ou après `delaiChemin` s (admin), l'hôte envoie `cm` → fondu noir → `changerCarte` (nouveaux départs, décor, objectif). Même carte ⇒ ancien comportement.
