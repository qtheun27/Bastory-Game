...
- `config.js` : `CONFIG_PAR_DEFAUT` (persos, armes, modes, maps, rôles, gadgets, quêtes, rangs, skins, `app` = réglages généraux) + `migrerConfig(c)` : mises à jour numérotées (`c.version`, actuellement **23** (les nouvelles listes de config — combos, météos, raretés, pass… — sont ajoutées automatiquement si absentes, via la liste de clés en tête de `migrerConfig`)). Toute nouvelle donnée ⇒ nouveau bloc `if ((c.version || 0) < N)` placé **après** le dernier.
